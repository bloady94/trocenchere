...
public class ServletTestJDBCImplInsert extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Utilisateur florian = new Utilisateur("corentinLeCaramel","ROBIN", "Corentin", "jadoreJPP@gmail.com", "0612121212", "rue des Blasés", "35000", "RENNES", "motdepasse", 0, true);
		...
		Categorie loisirs = new Categorie("Loisirs");
		
		ArticleVendu drogueDuBonheur = new ArticleVendu("LSD", "Petite pilule du bonheur :)", LocalDate.now(), LocalDate.of(2023, 4, 8), 100, 220, florian,  loisirs);
		ArticleVendu pendentif = new ArticleVendu("Pendentif", "Pendentif trop mignon en forme de coeur <3", LocalDate.now(), LocalDate.of(2023, 4, 29), 10, 50, florian,  loisirs);
		ArticleVendu twingo = new ArticleVendu("Twingo", "Sublime petite Twingo noire", LocalDate.now(), LocalDate.of(2023, 5, 15), 100, 350, florian,  auto);
		...
		try {
			dao.insert_utilisateur(florian);
			dao.insert_categorie(loisirs);
			dao.insert_categorie(auto);
			dao.insert_articleVendu(drogueDuBonheur);
			dao.insert_articleVendu(pendentif);
			dao.insert_articleVendu(twingo);
